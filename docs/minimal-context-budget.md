...
自动策略不执行硬重置。若摘要压缩无法完成，用户仍可手动执行 `/gateway-compact` 或 `/clear-context`。
...
两个命令在 minimal session 中都可以使用。`/gateway-compact` 需要 `dsh-compaction-basic` 包可解析；`/clear-context` 使用插件内的固定摘要器，不需要模型调用。
